...
int main(int argc, const char *argv[]) {
	...
	if (new_path == nullptr && new_base == nullptr) {
		cerr << "Check how OLD ELF symbols would be mapped to NEW" << endl << endl
	...
	cout << "\e[1mfile " << old_path << endl << "  -> " << new_path << "\e[0m" << endl << endl;
	for (const auto & sym : old_file.bean.symbols) {
		if (auto m = map.find(sym.address)) {
			sym.dump(cout, verbose, &old_file.bean.symbols, "\e[32msym ");
			const auto & m_sym = new_file.bean.symbols.floor(m->value);
			assert(m_sym);
			m_sym->dump(cout, verbose, &new_file.bean.symbols, "\e[0m -> ");
		} else {
			sym.dump(cout, verbose, &old_file.bean.symbols, "\e[31msym ");
		}
		cout << "\e[0m" << endl;
	}
}
